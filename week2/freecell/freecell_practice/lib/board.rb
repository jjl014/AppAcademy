...
class Board
  ...
  def render
    ...
    render_tableaus
    render_message
    print "cursor_pos  x:#{@cursor_pos[0]},#{@cursor_pos[1]}"
    print "\n"
    show_hand
  end
  ...
  def setup
    ...
    cards.shuffle!
    temp_tabs = Array.new(8) {Array.new}
    i = 0
    until cards.empty? do
      temp_tabs[i % 8] << cards.pop
      i += 1
    end
    8.times {|n| @tableaus[n] = Tableau.new(temp_tabs[n])}
  end
  ...
  def get_stack(tab_num, idx)
    if @tableaus[tab_num].count - idx > free_cells_count
      raise UserError.new("not enough freecells")
    end
    @hand = @tableaus[tab_num].grab(idx)
    set_message = ""
  ...
  def free_cells_count
    @freecells.count {|cell| cell.nil?}
  end

  def place_in_freecells(idx)
    raise UserError.new("invalid move") if hand.length > 1
    raise UserError.new("freecell is taken") unless @freecells[idx].nil?
    raise UserError.new("no more freecells left") if @freecells.none? {|cell| cell.nil?}
    card = @hand.first
    @freecells[idx] = card
    @hand = []
    set_message("")
  ...
  def get_freecell(idx)
    raise UserError.new("no card there") if @freecells[idx].nil?
    @hand = [@freecells[idx]]
    @freecells[idx] = nil
    set_message("")
  end

  def place_in_foundations(idx)
    raise UserError.new("only one card at a time") if @hand.length > 1
    card = @hand.first
    if @foundations[idx].empty? && card.value != :ace
      raise UserError.new("only aces can be placed in open foundation piles")
    end
    top_card = @foundations[idx].last
    unless (@foundations[idx].empty? && card.value == :ace) ||
      (top_card.num == card.num - 1 && top_card.suit == card.suit)
      raise UserError.new("card cannot be placed in that pile")
    end
    @foundations[idx] << card
    @hand = []
    set_message("")
  end
  ...
  def render_freecells
    4.times do |idx|
      print " "
      element = @freecells[idx]
      if @cursor_pos[0] == 0 && @cursor_pos[1] == idx
  ...
  def render_foundations
    4.times do |idx|
      print " "
      element = @foundations[idx]
      if @cursor_pos[0] == 0 && @cursor_pos[1] == idx + 4
  ...
  def render_tableaus
    ...
    print "\n"
    longest_len = lower_limit
    idx = 0
    while idx < longest_len
      8.times do |tab_num|
        print "  "
        card = @tableaus[tab_num][idx]
        if tab_num == y && idx + 1 == x
  ...
  def render_unit(card, bg = :white)
    if card.nil?
      print "   ".colorize(background: bg)
    else
      print card.display(bg)
    end
  end
